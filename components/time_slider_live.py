...
class TimeSliderLive:
    ...
    def _calculate_time_step(self, hours_per_second: float, updates_per_second: float) -> int:
        """
        ...
        if len(self.data) > 1:
            total_data_duration = (self.timestamps[-1] - self.timestamps[0]).total_seconds() / 3600
        ...
        if steps_to_advance > 0:
            st.session_state[f"{self.session_key}_accumulated_time"] = accumulated - (steps_to_advance * avg_hours_per_step)
    ...
    def render_controls(self) -> None:
        ...
        with col3:
            ...
            current_playing_state = st.session_state.get(f"{self.session_key}_is_playing", False)
        ...
        with col6:
            ...
            current_speed = st.session_state.get(f"{self.session_key}_speed_multiplier", 10.0)
    ...
    def run_simple_loop(self,
        ...
        """
        def default_content_renderer(idx: int, timestamp: pd.Timestamp, data_row: pd.Series, iteration: int):
