...
class TimeSlider:
    ...
    def __init__(self, data: pd.DataFrame, session_key: str = "time_slider"):
        ...
        self.formatted_timestamps = [self._format_timestamp(ts) for ts in self.timestamps]
    ...
    def render(self, 
               ...
               renders_per_second: float = 2.0) -> Tuple[int, pd.Timestamp]:
        """
        ...
        if self._is_autoplay_active() and len(self.data) >= 2:
            try:
                ...
                if hours_per_step > 0 and renders_per_second > 0:
                    ...
                    steps = int(st.session_state[f"{self.session_key}_accum"] / hours_per_step)
                    if steps > 0:
                        st.session_state[f"{self.session_key}_accum"] -= steps * hours_per_step
        ...
        if show_controls:
            ...
            with col5:
                autoplay_text = (
                    "⏸️ Stop" if self._is_autoplay_active() else "▶️ Play"
                )
                # Use a regular button check instead of callback to avoid timing issues
                if st.button(
                    autoplay_text,
                    key=f"{self.session_key}_autoplay_btn"
                ):
        ...
        if show_current_info:
            if self._is_autoplay_active():
                ...
                if len(self.data) > 10000:
                    dataset_size_info += " ⚠️"
                st.info(f"**Current Time:** {self._format_timestamp(current_timestamp)} | **Speed:** {hours_per_second}h/s | **Renders:** {renders_per_second}/s{dataset_size_info}")
            else:
                st.info(f"**Current Time:** {self._format_timestamp(current_timestamp)} | **Position:** {current_idx + 1}/{len(self.data)}")
